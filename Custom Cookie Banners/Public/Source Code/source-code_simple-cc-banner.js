...
// <script src="https://calbie-custom-scripts.vercel.app/simple-cc-banner.js" defer></script>

// user-preferences.js
document.addEventListener('DOMContentLoaded', () => {
  console.log("[Calbie Code] Simple Cookie Banner - Running");
  try {
    // --- DOM Elements ---
    const consentBanner = document.querySelector('[data-calbie-cc="banner"]');
    const acceptButton = document.querySelector('[data-calbie-cc="accept"]');
    const declineButton = document.querySelector('[data-calbie-cc="decline"]');
    ...
    if (!consentBanner) {
      console.warn(
        '[Calbie Code] Simple Cookie Banner - Warning: Banner element not found. Please add data-calbie-cc="banner" attribute to your banner div.'
      );
      return;
    }
    ...
    if (!acceptButton || !declineButton) {
      console.warn(
        '[Calbie Code] Simple Cookie Banner - Warning: Action buttons not found. Please check your data-calbie-cc attributes.'
      );
      return;
    }
    ...
    // --- Constants ---
    const CONSENT_KEY = 'calbie_cc_status';
    ...
    function hideBanner() {
      if (consentBanner) {
        consentBanner.style.display = 'none';
        console.log('[Calbie Code] Simple Cookie Banner - Info: Banner hidden');
      }
    }
    ...
    function showBanner() {
      if (consentBanner) {
        ...
        console.log('[Calbie Code] Simple Cookie Banner - Info: Banner shown');
      }
    }
    ...
    function slideInBanner() {
      ...
      console.log('[Calbie Code] Simple Cookie Banner - Info: Banner sliding in');
    }
    ...
    function slideOutBanner() {
      ...
      if (typeof consentBanner.animate === 'function') {
        ...
        anim.onfinish = function() {
          consentBanner.style.display = 'none';
          anim.cancel();
          console.log('[Calbie Code] Simple Cookie Banner - Info: Banner slid out');
        };
      } else {
        consentBanner.style.display = 'none';
      }
    ...
    /**
     * Unblocks existing scripts by restoring their functionality.
     */
    function unblockScripts() {
      try {
        // Restore Google Analytics / GTM dataLayer
        if (!window.dataLayer || typeof window.dataLayer.push !== 'function' || window.dataLayer._blocked) {
          ...
          console.log('[Calbie Code] Simple Cookie Banner - Success: Google Analytics/GTM unblocked');
        }

        // Restore gtag function if it was stubbed
        if (window.gtag && window.gtag._blocked) {
          delete window.gtag._blocked;
          console.log('[Calbie Code] Simple Cookie Banner - Success: gtag function unblocked');
        }

        // Restore Meta Pixel
        if (window.fbq && window.fbq._blocked) {
          delete window.fbq._blocked;
          console.log('[Calbie Code] Simple Cookie Banner - Success: Meta Pixel unblocked');
        }

        // Restore Google Maps
        if (window.initMap && window.initMap._blocked) {
          delete window.initMap._blocked;
          console.log('[Calbie Code] Simple Cookie Banner - Success: Google Maps unblocked');
        }
      } catch (e) {
        console.error("[Calbie Code] Simple Cookie Banner - Error unblocking scripts: ", e);
      }
    }

    /**
     * Blocks marketing and analytics scripts from executing.
     */
    function blockScripts() {
      try {
        // Block Google Analytics / GTM dataLayer
        window.dataLayer = {
          push: function () {
            console.log('[Calbie Code] Simple Cookie Banner - Info: GTM/GA dataLayer push blocked');
          },
          _blocked: true,
        };
        window.gtag = function () {};
        window.gtag._blocked = true;
        console.log('[Calbie Code] Simple Cookie Banner - Success: Google Analytics/GTM blocked');

        // Block Meta Pixel
        window.fbq = function () {};
        window.fbq._blocked = true;
        window._fbq = window.fbq;
        console.log('[Calbie Code] Simple Cookie Banner - Success: Meta Pixel blocked');

        // Block Google Maps
        window.initMap = function () {
          console.log('[Calbie Code] Simple Cookie Banner - Info: Google Maps initialization blocked');
        };
        window.initMap._blocked = true;
        console.log('[Calbie Code] Simple Cookie Banner - Success: Google Maps blocked');
      } catch (e) {
        console.error("[Calbie Code] Simple Cookie Banner - Error blocking scripts: ", e);
      }
    }

    /**
     * Manages script blocking based on consent status.
    ...
    if (acceptButton) {
      acceptButton.addEventListener('click', (e) => {
        try {
          ...
          console.log('[Calbie Code] Simple Cookie Banner - Success: Banner accepted');
          manageScripts('accepted');
        } catch (err) {
          console.error("[Calbie Code] Simple Cookie Banner - Error on accept: ", err);
        }
      });
    }
    ...
    if (declineButton) {
      declineButton.addEventListener('click', (e) => {
        try {
          ...
          console.log('[Calbie Code] Simple Cookie Banner - Success: Banner declined');
          manageScripts('declined');
        } catch (err) {
          console.error("[Calbie Code] Simple Cookie Banner - Error on decline: ", err);
        }
      });
    }
    ...
    if (openSettingsLink) {
      openSettingsLink.addEventListener('click', (e) => {
        try {
          ...
          console.error("[Calbie Code] Simple Cookie Banner - Error opening settings: ", err);
        }
      });
    }
    ...
    console.log('[Calbie Code] Simple Cookie Banner - Info: Detected scripts:', detectedScripts);
    
    // If no scripts detected, hide banner and exit
    if (!hasAnyScripts(detectedScripts)) {
      console.log('[Calbie Code] Simple Cookie Banner - Info: No scripts detected, hiding banner');
      hideBanner();
      return;
    }
    ...
    if (currentConsent) {
      ...
      console.log(`[Calbie Code] Simple Cookie Banner - Success: Loaded existing consent (${currentConsent})`);
    } else {
      setTimeout(function() {
        slideInBanner();
      }, BANNER_DELAY);
      console.log('[Calbie Code] Simple Cookie Banner - Info: No previous consent found, banner will appear after delay');
    }
    ...
    function loadGTM(gtmId) {
      ...
      console.log(`[Calbie Code] Simple Cookie Banner - Success: GTM loaded with ID: ${gtmId}`);
    }
    ...
    function enableGTM() {
      ...
      if (gtmId) {
        ...
        if (window.google_tag_manager) {
          window.dataLayer = window.dataLayer || [];
          console.log(`[Calbie Code] Simple Cookie Banner - Info: GTM already loaded (${gtmId})`);
        } else {
    ...
    function blockGTM() {
      window.dataLayer = {
        push: function () {
          console.log('[Calbie Code] Simple Cookie Banner - Info: GTM dataLayer push blocked');
        },
      };
      console.log('[Calbie Code] Simple Cookie Banner - Success: GTM blocked');
    }

    console.log("[Calbie Code] Simple Cookie Banner - Success: Initialized");
  } catch (error) {
    console.error("[Calbie Code] Simple Cookie Banner - Error: ", error);
  }
});
